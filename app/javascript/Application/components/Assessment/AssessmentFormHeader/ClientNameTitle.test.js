...
describe('ClientNameTitle', () => {
  const render = ({ firstName, middleName, lastName, suffix, dob, estimatedDob } = {}) =>
    shallow(
      <ClientNameTitle
        firstName={firstName}
  ...
  describe('with a full name', () => {
    it('renders the full name in proper formatting', () => {
      const wrapper = render({
        ...
        suffix: 'Jr',
        dob: '2018-01-01',
        estimatedDob: '2018-01-01',
      })
      expect(wrapper.find('.card-title-block #child-name').text()).toBe('Simpson, Homer Jay, Jr')
    })
  })
})
